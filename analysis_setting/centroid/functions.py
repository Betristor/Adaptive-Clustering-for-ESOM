...
def cluster(settings, data, cluster_log=False):
    ...
    if 'centroid' in settings and settings['centroid'] and settings['trial'] != 'simulated':
...
def run_trials(config, wind_all, pv_all, load_all, expected, features):
    ...
    if settings['normalize']:
        for trial in trials:
            scaler = MinMaxScaler()
            temp = scaler.fit_transform(data[trial])
            data[trial] = pd.DataFrame(temp)
        ...
        for trial in trials:
            for method in methods:
                for centroid in [True, False]:
                    ...
                    print('{}th trial: '.format(i),round(result['elapsed_time'],2),'s')
